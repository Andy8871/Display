...
/* 下面几个函数实现用DES算法加密和解密*/
void  stktranspose(char *data,int *t,int n)/*替代*/
{
	char x[64];
	int i;
...
void  stkrotate(char *x,int s)/*移位*/
{
	char x0,x1,x2,x3;
...
void  stkf(char *key,char *a,char *x)	/*  迭代  */
{
	int r,k,j;
	char e[48],y[48];
...
short  stkAscToHex(char * Hex, char * asc, int AscLen)
{
	char * HexPtr = Hex;
	short i;
	for(i = 0; i < AscLen; i++)
...
//        errlog(0, Pin);
	...
	stkAscToHex((char*)zpk, (char*)Zpk, 16);
...
//        errlog(0, Oldpin);
//        errlog(0, Zpk);
        stkAscToHex((char *)oldpin, (char *)Oldpin, 16); 
	stkAscToHex((char*)zpk, (char*)Zpk, 16);
...
short stkGenMac(char *mac_data,int len,char *zak,char *mac)
{
	...
	int i,j;
        xDataptr=xData;  
	stkAscToHex((char*)Kmac, (char*)zak, 16);
